...
//
// We deliberately DO NOT exercise the real CV pipeline here.
// Integration tests already cover the happy + error paths of
// POST /api/v1/watches/:id/readings/verified against a fake dial
// reader. E2E against the preview Worker runs with the real CV
// container, which we can't mock, and the `verified_reading_cv`
// feature flag is default-off for freshly-registered CI users
// anyway.
//
// What we DO exercise:
...
// the error mapper — is connected to the real backend. The CV path
...
// the flag gate rejects us LONG before the dial reader ever looks
// at the file. Embedded as base64 + decoded at test-time so no
// fixture file ships with the repo.
// ------------------------------------------------------------------
...
test("register → add watch → verified-reading flow renders and surfaces flag-off copy", async ({
  ...
  // The preview Worker shares the production `verified_reading_cv`
  // flag. Depending on whether the flag is currently rolled out
  // and how the CV pipeline handles a 1×1 PNG, the SPA can land in
  // one of three terminal states:
  //
  //   (a) flag off → 503 → role=alert: "verified readings aren't
  //       enabled" / "verified-reading is temporarily unavailable"
  //   (b) flag on, container rejects (no_dial_found /
  //       unsupported_dial / low_confidence / malformed_image) →
  //       422 → role=alert: rejection-specific copy
  //   (c) flag on, container reports a confident enough read on
  //       the 1×1 fixture → 201 → role=status: "Saved. Dial read
  //       at HH:MM:SS, ..."
  //
  // The CV pipeline rejects single-pixel inputs deterministically
  // (no_dial_found path); the smoke only checks "the submit round-
  // trip surfaces a human-readable outcome", which is true
  // regardless of which path the preview lands on.
  //
  // If we ever need to assert flag-specific behaviour, switch this
  // to an integration test with a faked dial reader rather than an
