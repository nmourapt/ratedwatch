...
| **Dial reader**               | The CV pipeline that turns a verified-reading photo into a structured `displayed_time: { h, m, s } + confidence` triple. Runs as a Cloudflare Container at `container/dial-reader/`, called from the Worker via the typed adapter at `src/domain/dial-reader/`. Sole verified-reading backend after slice #11 of PRD #73. |
...
- **CV dial reader (Python container):** Lives at `container/dial-reader/` and runs as a Cloudflare Container behind the `DIAL_READER` Durable Object binding. **It is the sole verified-reading backend** — the legacy Workers AI runner was deleted in slice #11 of PRD #73 (cutover). Toolchain is **uv** (venv + lockfile management), **FastAPI + uvicorn** for the HTTP surface, **pytest + ruff + mypy** for tests/lint/types. Runtime deps include OpenCV-headless, numpy, Pillow, pillow-heif, sentry-sdk. The Worker side calls it via the typed adapter at `src/domain/dial-reader/` (`readDial(image, env)` returning a `DialReadResult` discriminated union — `success` / `rejection` / `malformed_image` / `transport_error`). Tests use a `__setTestDialReader` module-level override (same pattern as `__setTestExifReader` / `__setTestRateLimiter`). Container CI lives at `.github/workflows/python-ci.yml` and is path-filtered to `container/dial-reader/**`. Any change under that subtree must keep `cd container/dial-reader && uv run pytest && uv run ruff check . && uv run mypy src/` green; CI runs the same trio with `uv sync --frozen`. The verified-reading flow itself is gated by the `verified_reading_cv` feature flag (KV key in the `FLAGS` namespace; renamed from the legacy `ai_reading_v2` in slice #11 — the backward-compat fallback that read the legacy key was removed in the post-cutover cleanup PR, so this is the only key to set).
