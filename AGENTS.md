...
- **Secrets:** `wrangler secret put` for Worker-side secrets; `.dev.vars` for local dev (gitignored). Never hardcode. Required production secrets:
  ...
  - `READING_TOKEN_SECRET` — HMAC-SHA256 secret used to sign the `reading_token` envelope exchanged between `POST /readings/verified/draft` and `POST /readings/verified/confirm` (slice #6 of PRD #99). Generate with `openssl rand -base64 32` (32+ bytes of entropy required); set with `wrangler secret put READING_TOKEN_SECRET` **before** any `wrangler deploy` that ships the verified-reading two-step flow. **Missing secret is fatal:** every `/confirm` request returns 401 because token verification can't compute the signature. Tests seed a deterministic value via the miniflare bindings (`vitest.config.ts`).
...
- **D1 migrations:** Live at `migrations/NNNN_*.sql`. The `deploy` job in `.github/workflows/ci.yml` runs `wrangler d1 migrations apply rated-watch-db --remote` BEFORE `wrangler deploy --env=production` on every push to `main`, so a PR that adds a migration ships it as part of the auto-deploy. Tests use miniflare which auto-applies migrations via `vitest.config.ts`; preview deploys use the real D1, so a brand-new migration won't be present until the auto-deploy runs (the `e2e-smoke` job exercises the real preview against pre-merge schema — flag this in the PR body if a slice depends on the new column). For manual application before/after a deploy run `wrangler d1 migrations apply rated-watch-db --remote` locally with `.env` sourced.
- **Verified-reading dial reader (two-step API):** As of slice #6 of PRD #99 (issue #105), the verified-reading flow is split into two HTTP endpoints:
  1. `POST /api/v1/watches/:id/readings/verified/draft` — accepts the photo (multipart `image`), runs the VLM pipeline (cropper → reader → deviation calc against EXIF anchor), persists the photo to `drafts/{user_id}/{uuid}.jpg` in the `WATCH_IMAGES` R2 bucket, and returns a signed `reading_token` + the predicted MM:SS + a photo URL + the server-clock hour. **Does not save a reading row.**
  2. `POST /api/v1/watches/:id/readings/verified/confirm` — accepts `{ reading_token, final_mm_ss, is_baseline? }`. Validates the token (HMAC-SHA256 signature, 5-minute expiry, payload `user_id`/`watch_id` match the request session/URL), enforces the per-click `±30 s` adjustment cap on the [0, 3600) MM:SS circle, saves the reading row, and moves the photo from `drafts/` to `verified/{user_id}/{reading_id}.jpg`. **Anti-cheat property: `/draft` never returns the deviation — the SPA confirmation page (slice #7) lets the user adjust ± seconds without seeing the deviation it would produce, and the server enforces the same ±30 s cap so a malicious client can't bypass the UI.** Larger corrections require retake.
     The `reading_token` envelope (`src/domain/reading-token/`) is `base64url(JSON).base64url(HMAC-SHA256(payload, secret))` — chosen over JWT because the payload is application-specific, verification is HMAC-only, and the attack surface is narrower than a JWT lib's. `READING_TOKEN_SECRET` is a Worker secret (`wrangler secret put READING_TOKEN_SECRET` with 32+ bytes from `openssl rand -base64 32`). The R2 lifecycle rule on the `drafts/` prefix expires abandoned drafts after 24h (well past the 5-minute token window) — see `infra/terraform/r2.tf`.
     Pipeline internals (unchanged from slice #4): `src/domain/dial-cropper/` does JS HoughCircles dial detection + `env.IMAGES` (Cloudflare Images) for HEIC decode / EXIF rotation / 1024-px-long-edge resize / final 768×768 crop. `src/domain/dial-reader-vlm/` does a single VLM read against `openai/gpt-5.2` via Cloudflare AI Gateway, prompted with chain-of-thought hand-identification + an EXIF anchor (sanity check, not echoed). The orchestrator at `src/domain/reading-verifier/verifier.ts` resolves the reference timestamp from EXIF DateTimeOriginal (bounded ±5min/+1min vs server arrival; falls back to server arrival when EXIF is missing), runs the cropper + reader, and computes a signed MM:SS-modulo-30min deviation against the reference clock. Single VLM call for now; median-of-3 + the anchor-disagreement guard land in slice #5 of PRD #99. The bake-off harness lives at `scripts/vlm-bakeoff/` and is the canonical reference for prompt/crop/model tuning. The `verified_reading_cv` flag in KV is still gating this in production at `mode:never` until slice #10's rollout. The `manual_with_photo` companion route remains a 503 stub until a later slice of PRD #99 rebuilds it. The previous Python `container/dial-reader/` subtree, the `DIAL_READER` Durable Object binding, the `VERIFIED_READING_LIMITER` ratelimit, the `@/domain/dial-reader/` Worker adapter, and the `dial_reader_*` Analytics Engine events were all removed in slice #1 of PRD #99 (issue #100). Slice #4 (issue #103) wired the VLM pipeline back into the verified-reading route; slice #6 (issue #105) split it into the draft/confirm two-step API.
...
- CI: GitHub Actions — `verify` (typecheck, build, unit + integration tests, coverage) → on `pull_request`: `preview` (Wrangler `versions upload --preview-alias=pr-<N>`) + `e2e-smoke` (Playwright against the preview URL); on `push: main`: `deploy` (D1 migrations + `wrangler deploy --env=production`). The deploy job is in `.github/workflows/ci.yml`, gated on `verify` passing on the same SHA, with a `production-deploy` concurrency group (queues, never cancels). Manual `workflow_dispatch` from the Actions UI re-runs the deploy without a no-op commit. Rollback: `wrangler versions list --name=ratedwatch` then `wrangler versions deploy <previous-version-id> --name=ratedwatch`. For a feature-flag rollback (e.g. yank verified-reading on a bad VLM rollout), use `wrangler kv key put verified_reading_cv '{"mode":"never"}' --namespace-id=b4298bd1887d4dae8d1c113ec99cea56 --remote` — Worker stays deployed, the feature just goes off.
...
5. PR before merge. Merge to `main` triggers an auto-deploy via `.github/workflows/ci.yml` (D1 migrations → `wrangler deploy --env=production`). For an emergency manual deploy, run `npm run deploy` locally — but prefer letting CI deploy from a clean checkout so a stale local `dist/` can't ship (this happened once on 2026-04-30 — local `dist/` was 1.5h old, deploy uploaded the OLD SPA bundle alongside the NEW Worker, breaking the page).
