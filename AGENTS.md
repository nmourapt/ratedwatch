...
- **Verified-reading dial reader:** being replaced with a Worker-side VLM hybrid (PRD #99). Until that lands, the verified-reading endpoints (`POST /api/v1/watches/:id/readings/verified` and `POST /api/v1/watches/:id/readings/manual_with_photo`) return 503 with `{ error_code: "verified_readings_disabled" }` — the SPA's existing error mapper in `src/app/watches/verifiedReadingErrors.ts` renders that as a friendly "Verified readings aren't enabled for your account yet" alert, and the `verified_reading_cv` feature flag (KV `FLAGS` namespace) stays at `mode:never` in production. The previous Python `container/dial-reader/` subtree, the `DIAL_READER` Durable Object binding, the `VERIFIED_READING_LIMITER` ratelimit, the `@/domain/dial-reader/` Worker adapter, and the `dial_reader_*` Analytics Engine events were all removed in slice #1 of PRD #99 (issue #100). Slice #4 (issue #103) reintroduces a Worker-only VLM pipeline.
